...
def parse_head_annotation_with_id(annotation_path, image_width, image_height):  
    """  
    解析YOLO格式的标注文件，返回标注框的列表。  
    标注文件的格式是每行一个标注框，格式为 "label x_center y_center width height"。  
    这里假设坐标是归一化的，如果需要像素坐标，则需要在函数内部进行转换。  
    ...
    with open(annotation_path, 'r') as file:  
        ...
        for line in lines:  
            ...
            # 将调整后的边界框添加到列表中  
            boxes.append((label,x_min_px, y_min_px, width_px, height_px))  
    return boxes  
  
def cut_and_save_head_images_with_id(image_path, annotation_path, output_folder):  
    """  
    ...
    boxes = parse_head_annotation_with_id(annotation_path, image_width, image_height)  

    if len(boxes)!=1:
        return
    for idx, (label,x_min, y_min, width, height) in enumerate(boxes):  
        id=image_path.split('/')[-1].split('.')[0].split('_')[0]
...
def parse_annotation_without_id(annotation_path, image_width, image_height):  
    """  
    解析YOLO格式的标注文件，返回标注框的列表。  
    标注文件的格式是每行一个标注框，格式为 "label x_center y_center width height"。  
    这里假设坐标是归一化的，如果需要像素坐标，则需要在函数内部进行转换。  
...
def cut_and_save_images_without_id(image_path, annotation_path, output_folder):  
    """  
    根据YOLO格式的标注文件切割图像并保存到输出文件夹。  
    """  
    image = Image.open(image_path)  
    ...
    boxes = parse_annotation_without_id(annotation_path, image_width, image_height)  
      
    for idx, (label, x_min, y_min, width, height) in enumerate(boxes):  
        # 切割图像  
        if label==3:
            if not os.path.exists(output_folder+'/'+str(idx)):
                os.makedirs(output_folder+'/'+str(idx))
            cut_image = image.crop((x_min, y_min, x_min + width, y_min + height))  
            
            # 构造输出文件名并保存切割后的图像
            base_name = os.path.splitext(os.path.basename(image_path))[0]
            output_filename = f"{idx}_{base_name}_{label}.jpg"  
            output_path = os.path.join(output_folder+'/'+str(idx), output_filename)
            cut_image.save(output_path)  
            print(f"Saved {output_path}")  
        ...
        for dir in dirs:
            ...
            for filename in os.listdir(img_dir):  
                if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.gif')):  
                    ...
                    annotation_dir= os.path.join('Market/head/train/train_all_head_labels/head_labels', dir)
                    ...
                    if os.path.exists(annotation_path):  
                        ...
                        if head:
                            if with_id:
                                cut_and_save_head_images_with_id(image_path, annotation_path, output_folder)
                            else:
                                cut_and_save_images_without_id(image_path, annotation_path, output_folder)
                        else:
                            if with_id:
...
input_folder = 'Market/pytorch/train_all'  # 图像文件夹路径  ReIDDataset/CUHKSYSU/images
output_folder = '../head'  # 输出文件夹

# 处理图像
process_images_in_folder(input_folder, output_folder,head=True,with_id=True)